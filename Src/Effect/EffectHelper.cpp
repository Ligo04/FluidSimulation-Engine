...
#define EFFECTHELPER_CREATE_SHADER(FullShaderType, ShaderType) \
    { \
        ...
        hr                                  = device->Create##FullShaderType(blob->GetBufferPointer(), \
                                            blob->GetBufferSize(), \
                                            nullptr, \
                                            m_##FullShaderType##s[nameID]->p##ShaderType.GetAddressOf()); \
        break; \
    }
    ...
    { \
        ...
        { \
            ...
            { \
                ...
                if (pCBData) \
                { \
                    pEffectPass->p##ShaderType##ParamData = std::make_unique<CBufferData>( \
                        pCBData->cbufferName.c_str(), pCBData->startSlot, (uint32_t)pCBData->cbufferData.size()); \
...
#define EFFECTHELPER_SET_SHADER_DEBUG_NAME(FullShaderType, ShaderType) \
    { \
        ...
        { \
            ...
            it.second->p##ShaderType->SetPrivateData( \
                WKPDID_D3DDebugObjectName, (uint32_t)name##ShaderType.size(), name##ShaderType.c_str()); \
        } \
    }
    ...
    { \
        ...
        { \
            ...
            { \
                ...
                memcpy_s(p##ShaderType##Info->pParamData->cbufferData.data(), \
                         p##ShaderType##ParamData->cbufferData.size(), \
                         p##ShaderType##ParamData->cbufferData.data(), \
                         p##ShaderType##ParamData->cbufferData.size()); \
            ...
            deviceContext->ShaderType##SetConstantBuffers(p##ShaderType##Info->pParamData->startSlot, \
                                                          1, \
                                                          p##ShaderType##Info->pParamData->cBuffer.GetAddressOf()); \
        } \
    }
    ...
    { \
        ...
        { \
            ...
            { \
                ...
                if (count == 1) \
                { \
                    deviceContext->ShaderType##SetShaderResources( \
                        slot, 1, shaderResources.at(slot).pSRV.GetAddressOf()); \
                    ++slot, mask >>= 1; \
                } \
                else \
                { \
...
struct ConstantBufferVariable : public IEffectConstantBufferVariable
{
    ...
    void SetUInt(uint32_t val) override { SetRaw(&val, 0, 4); }

    void SetSInt(int val) override { SetRaw(&val, 0, 4); }
    ...
    struct PropertyFunctor
    {
        ...
        void operator()(const DirectX::XMFLOAT4X4& val)
        {
            cbv.SetFloatMatrix(4, 4, reinterpret_cast<const float*>(&val));
        }
    ...
    void Set(const Property& prop) override { std::visit(PropertyFunctor(*this), prop); }
...
{
    EffectPass(EffectHelper*                                 _pEffectHelper,
        ...
        pEffectHelper(_pEffectHelper),
        passName(_passName), cBuffers(_cBuffers), shaderResources(_shaderResources), samplers(_samplers),
        rwResources(_rwResources)
    {}
    ~EffectPass() override {}
...
public:
    ...
    // 更新收集着色器反射信息
    HRESULT UpdateShaderReflection(std::string_view        name,
                                   ID3D11Device*           device,
    ...
    void Clear();
    // 根据Blob创建着色器并指定标识名
    HRESULT CreateShaderFromBlob(std::string_view name, ID3D11Device* device, uint32_t shaderFlag, ID3DBlob* blob);
...
HRESULT EffectHelper::Impl::UpdateShaderReflection(std::string_view        name,
                                                   ID3D11Device*           device,
                                                   ID3D11ShaderReflection* pShaderReflection,
                                                   uint32_t                shaderFlag)
{
    ...
    {
        // 获取线程组维度
        pShaderReflection->GetThreadGroupSize(&m_ComputeShaders[nameID]->threadGroupSizeX,
    ...
    for (uint32_t i = 0;; ++i)
    {
        ...
        if (sibDesc.Type == D3D_SIT_CBUFFER)
        {
            ...
            if (!isParam)
            {
                ...
                {
                    m_CBuffers.emplace(std::make_pair(
                        sibDesc.BindPoint, CBufferData(sibDesc.Name, sibDesc.BindPoint, cbDesc.Size, nullptr)));
                ...
                if (cbDesc.Variables > 0)
                {
                    ...
                    {
                        case VertexShader:
                            m_VertexShaders[nameID]->cbUseMask |= (1 << sibDesc.BindPoint);
                            break;
                        case DomainShader:
                            m_DomainShaders[nameID]->cbUseMask |= (1 << sibDesc.BindPoint);
                            break;
                        case HullShader:
                            m_HullShaders[nameID]->cbUseMask |= (1 << sibDesc.BindPoint);
                            break;
                        case GeometryShader:
                            m_GeometryShaders[nameID]->cbUseMask |= (1 << sibDesc.BindPoint);
                            break;
                        case PixelShader:
                            m_PixelShaders[nameID]->cbUseMask |= (1 << sibDesc.BindPoint);
                            break;
                        case ComputeShader:
                            m_ComputeShaders[nameID]->cbUseMask |= (1 << sibDesc.BindPoint);
                            break;
                    }
                }
            }
            else if (cbDesc.Variables > 0)
            {
                ...
                {
                    case VertexShader:
                        m_VertexShaders[nameID]->pParamData =
                            std::make_unique<CBufferData>(sibDesc.Name, sibDesc.BindPoint, cbDesc.Size, nullptr);
                        break;
                    case DomainShader:
                        m_DomainShaders[nameID]->pParamData =
                            std::make_unique<CBufferData>(sibDesc.Name, sibDesc.BindPoint, cbDesc.Size, nullptr);
                        break;
                    case HullShader:
                        m_HullShaders[nameID]->pParamData =
                            std::make_unique<CBufferData>(sibDesc.Name, sibDesc.BindPoint, cbDesc.Size, nullptr);
                        break;
                    case GeometryShader:
                        m_GeometryShaders[nameID]->pParamData =
                            std::make_unique<CBufferData>(sibDesc.Name, sibDesc.BindPoint, cbDesc.Size, nullptr);
                        break;
                    case PixelShader:
                        m_PixelShaders[nameID]->pParamData =
                            std::make_unique<CBufferData>(sibDesc.Name, sibDesc.BindPoint, cbDesc.Size, nullptr);
                        break;
                    case ComputeShader:
                        m_ComputeShaders[nameID]->pParamData =
                            std::make_unique<CBufferData>(sibDesc.Name, sibDesc.BindPoint, cbDesc.Size, nullptr);
                        break;
                }
            }
            ...
            for (uint32_t j = 0; j < cbDesc.Variables; ++j)
            {
                ...
                if (isParam)
                {
                    ...
                    {
                        case VertexShader:
                            m_VertexShaders[nameID]->params[svNameID] =
                                std::make_shared<ConstantBufferVariable>(svDesc.Name,
                                                                         svDesc.StartOffset,
                                                                         svDesc.Size,
                                                                         m_VertexShaders[nameID]->pParamData.get());
                            break;
                        case DomainShader:
                            m_DomainShaders[nameID]->params[svNameID] =
                                std::make_shared<ConstantBufferVariable>(svDesc.Name,
                                                                         svDesc.StartOffset,
                                                                         svDesc.Size,
                                                                         m_DomainShaders[nameID]->pParamData.get());
                            break;
                        case HullShader:
                            m_HullShaders[nameID]->params[svNameID] = std::make_shared<ConstantBufferVariable>(
                                svDesc.Name, svDesc.StartOffset, svDesc.Size, m_HullShaders[nameID]->pParamData.get());
                            break;
                        case GeometryShader:
                            m_GeometryShaders[nameID]->params[svNameID] =
                                std::make_shared<ConstantBufferVariable>(svDesc.Name,
                                                                         svDesc.StartOffset,
                                                                         svDesc.Size,
                                                                         m_GeometryShaders[nameID]->pParamData.get());
                            break;
                        case PixelShader:
                            m_PixelShaders[nameID]->params[svNameID] = std::make_shared<ConstantBufferVariable>(
                                svDesc.Name, svDesc.StartOffset, svDesc.Size, m_PixelShaders[nameID]->pParamData.get());
                            break;
                        case ComputeShader:
                            m_ComputeShaders[nameID]->params[svNameID] =
                                std::make_shared<ConstantBufferVariable>(svDesc.Name,
                                                                         svDesc.StartOffset,
                                                                         svDesc.Size,
                                                                         m_ComputeShaders[nameID]->pParamData.get());
                            break;
                    }
                }
                // 常量缓冲区的成员
        ...
        // 着色器资源
        else if (sibDesc.Type == D3D_SIT_TEXTURE || sibDesc.Type == D3D_SIT_STRUCTURED ||
                 sibDesc.Type == D3D_SIT_BYTEADDRESS || sibDesc.Type == D3D_SIT_TBUFFER)
        {
            auto it = m_ShaderResources.find(sibDesc.BindPoint);
            if (it == m_ShaderResources.end())
            {
                m_ShaderResources.emplace(
                    std::make_pair(sibDesc.BindPoint, ShaderResource {sibDesc.Name, sibDesc.Dimension, nullptr}));
            }
            ...
            {
                case VertexShader:
                    m_VertexShaders[nameID]->srUseMasks[sibDesc.BindPoint / 32] |= (1 << (sibDesc.BindPoint % 32));
                    break;
                case DomainShader:
                    m_DomainShaders[nameID]->srUseMasks[sibDesc.BindPoint / 32] |= (1 << (sibDesc.BindPoint % 32));
                    break;
                case HullShader:
                    m_HullShaders[nameID]->srUseMasks[sibDesc.BindPoint / 32] |= (1 << (sibDesc.BindPoint % 32));
                    break;
                case GeometryShader:
                    m_GeometryShaders[nameID]->srUseMasks[sibDesc.BindPoint / 32] |= (1 << (sibDesc.BindPoint % 32));
                    break;
                case PixelShader:
                    m_PixelShaders[nameID]->srUseMasks[sibDesc.BindPoint / 32] |= (1 << (sibDesc.BindPoint % 32));
                    break;
                case ComputeShader:
                    m_ComputeShaders[nameID]->srUseMasks[sibDesc.BindPoint / 32] |= (1 << (sibDesc.BindPoint % 32));
                    break;
            }
        }
        // 采样器
        else if (sibDesc.Type == D3D_SIT_SAMPLER)
        {
            ...
            {
                m_Samplers.emplace(std::make_pair(sibDesc.BindPoint, SamplerState {sibDesc.Name, nullptr}));
            }
            ...
            {
                case VertexShader:
                    m_VertexShaders[nameID]->ssUseMask |= (1 << sibDesc.BindPoint);
                    break;
                case DomainShader:
                    m_DomainShaders[nameID]->ssUseMask |= (1 << sibDesc.BindPoint);
                    break;
                case HullShader:
                    m_HullShaders[nameID]->ssUseMask |= (1 << sibDesc.BindPoint);
                    break;
                case GeometryShader:
                    m_GeometryShaders[nameID]->ssUseMask |= (1 << sibDesc.BindPoint);
                    break;
                case PixelShader:
                    m_PixelShaders[nameID]->ssUseMask |= (1 << sibDesc.BindPoint);
                    break;
                case ComputeShader:
                    m_ComputeShaders[nameID]->ssUseMask |= (1 << sibDesc.BindPoint);
                    break;
            }
        }
        // 可读写资源
                 ...
                 sibDesc.Type == D3D_SIT_UAV_APPEND_STRUCTURED || sibDesc.Type == D3D_SIT_UAV_CONSUME_STRUCTURED ||
                 sibDesc.Type == D3D_SIT_UAV_RWBYTEADDRESS)
        {
            ...
            if (it == m_RWResources.end())
            {
                m_RWResources.emplace(std::make_pair(sibDesc.BindPoint,
                                                     RWResource {sibDesc.Name,
                                                                 static_cast<D3D11_UAV_DIMENSION>(sibDesc.Dimension),
                                                                 nullptr,
                                                                 0,
                                                                 sibDesc.Type == D3D_SIT_UAV_RWSTRUCTURED_WITH_COUNTER,
                                                                 false}));
            ...
            {
                case PixelShader:
                    m_PixelShaders[nameID]->rwUseMask |= (1 << sibDesc.BindPoint);
                    break;
                case ComputeShader:
                    m_ComputeShaders[nameID]->rwUseMask |= (1 << sibDesc.BindPoint);
                    break;
            }
        }
    }
...
HRESULT EffectHelper::Impl::CreateShaderFromBlob(std::string_view name,
                                                 ID3D11Device*    device,
                                                 uint32_t         shaderFlag,
                                                 ID3DBlob*        blob)
{
    ...
    {
        case PixelShader:
            EFFECTHELPER_CREATE_SHADER(PixelShader, PS);
        case VertexShader:
            EFFECTHELPER_CREATE_SHADER(VertexShader, VS);
        case GeometryShader:
            EFFECTHELPER_CREATE_SHADER(GeometryShader, GS);
        case HullShader:
            EFFECTHELPER_CREATE_SHADER(HullShader, HS);
        case DomainShader:
            EFFECTHELPER_CREATE_SHADER(DomainShader, DS);
        case ComputeShader:
            EFFECTHELPER_CREATE_SHADER(ComputeShader, CS);
    }
...
EffectHelper::EffectHelper() : pImpl(std::make_unique<EffectHelper::Impl>()) {}
...
HRESULT EffectHelper::CreateShaderFromFile(std::string_view        shaderName,
                                           ...
                                           ID3DBlob**              ppShaderByteCode)
{
    ...
    if (memcmp(pBlobIn->GetBufferPointer(), dxbc_header, sizeof dxbc_header))
    {
        ...
        hr                        = D3DCompile(pBlobIn->GetBufferPointer(),
                        pBlobIn->GetBufferSize(),
                        filenameu8str.c_str(),
                        pDefines,
...
HRESULT EffectHelper::CompileShaderFromFile(std::wstring_view       filename,
                                            LPCSTR                  entryPoint,
                                            LPCSTR                  shaderModel,
                                            ID3DBlob**              ppShaderByteCode,
                                            ...
                                            ID3DInclude*            pInclude)
{
    ...
    return D3DCompileFromFile(
        filename.data(), pDefines, pInclude, entryPoint, shaderModel, dwShaderFlags, 0, ppShaderByteCode, ppErrorBlob);
}

HRESULT EffectHelper::AddGeometryShaderWithStreamOutput(std::string_view      name,
                                                        ID3D11Device*         device,
                                                        ID3D11GeometryShader* gsWithSO,
                                                        ID3DBlob*             blob)
{
...
void EffectHelper::Clear() { pImpl->Clear(); }
...
HRESULT EffectHelper::AddEffectPass(std::string_view effectPassName, ID3D11Device* device, const EffectPassDesc* pDesc)
{
    ...
    auto& pEffectPass = pImpl->m_EffectPasses[effectPassID] = std::make_shared<EffectPass>(
        this, effectPassName, pImpl->m_CBuffers, pImpl->m_ShaderResources, pImpl->m_Samplers, pImpl->m_RWResources);
...
{
    auto it = std::find_if(pImpl->m_Samplers.begin(),
                           pImpl->m_Samplers.end(),
                           [name](const std::pair<uint32_t, SamplerState>& p) { return p.second.name == name; });
    if (it != pImpl->m_Samplers.end())
        it->second.pSS = samplerState;
}
...
{
    auto it = std::find_if(pImpl->m_ShaderResources.begin(),
                           pImpl->m_ShaderResources.end(),
                           [name](const std::pair<uint32_t, ShaderResource>& p) { return p.second.name == name; });
    if (it != pImpl->m_ShaderResources.end())
        it->second.pSRV = srv;
}
...
void EffectHelper::SetUnorderedAccessByName(std::string_view           name,
                                            ID3D11UnorderedAccessView* uav,
                                            uint32_t*                  pInitialCount)
{
    auto it = std::find_if(pImpl->m_RWResources.begin(),
                           pImpl->m_RWResources.end(),
                           [name](const std::pair<uint32_t, RWResource>& p) { return p.second.name == name; });
    if (it != pImpl->m_RWResources.end())
    {
        it->second.pUAV = uav;
        if (pInitialCount)
...
void EffectPass::SetRasterizerState(ID3D11RasterizerState* pRS) { pRasterizerState = pRS; }
...
std::shared_ptr<IEffectConstantBufferVariable> EffectPass::VSGetParamByName(std::string_view paramName)
{
    if (pVSInfo)
    {
        ...
        if (it != pVSInfo->params.end())
            return std::make_shared<ConstantBufferVariable>(
                paramName, it->second->startByteOffset, it->second->byteWidth, pVSParamData.get());
    }
    return nullptr;
}
...
std::shared_ptr<IEffectConstantBufferVariable> EffectPass::DSGetParamByName(std::string_view paramName)
{
    if (pDSInfo)
    {
        ...
        if (it != pDSInfo->params.end())
            return std::make_shared<ConstantBufferVariable>(
                paramName, it->second->startByteOffset, it->second->byteWidth, pDSParamData.get());
    }
    return nullptr;
}
...
std::shared_ptr<IEffectConstantBufferVariable> EffectPass::HSGetParamByName(std::string_view paramName)
{
    if (pHSInfo)
    {
        ...
        if (it != pHSInfo->params.end())
            return std::make_shared<ConstantBufferVariable>(
                paramName, it->second->startByteOffset, it->second->byteWidth, pHSParamData.get());
    }
    return nullptr;
}
...
std::shared_ptr<IEffectConstantBufferVariable> EffectPass::GSGetParamByName(std::string_view paramName)
{
    if (pGSInfo)
    {
        ...
        if (it != pGSInfo->params.end())
            return std::make_shared<ConstantBufferVariable>(
                paramName, it->second->startByteOffset, it->second->byteWidth, pGSParamData.get());
    }
    return nullptr;
}
...
std::shared_ptr<IEffectConstantBufferVariable> EffectPass::PSGetParamByName(std::string_view paramName)
{
    if (pPSInfo)
    {
        ...
        if (it != pPSInfo->params.end())
            return std::make_shared<ConstantBufferVariable>(
                paramName, it->second->startByteOffset, it->second->byteWidth, pPSParamData.get());
    }
    return nullptr;
}
...
std::shared_ptr<IEffectConstantBufferVariable> EffectPass::CSGetParamByName(std::string_view paramName)
{
    if (pCSInfo)
    {
        ...
        if (it != pCSInfo->params.end())
            return std::make_shared<ConstantBufferVariable>(
                paramName, it->second->startByteOffset, it->second->byteWidth, pCSParamData.get());
    }
    return nullptr;
}

EffectHelper* EffectPass::GetEffectHelper() { return pEffectHelper; }

const std::string& EffectPass::GetPassName() { return passName; }
