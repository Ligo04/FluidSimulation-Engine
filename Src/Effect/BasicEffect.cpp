...
BasicEffect::BasicEffect(BasicEffect&& moveFrom) noexcept { pImpl.swap(moveFrom.pImpl); }
...
bool BasicEffect::InitAll(ID3D11Device* device)
{
    ...
    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("BasicInstance_VS",
                                                    shader_path + L"BasicInstance_VS.hlsl ",
                                                    device,
                                                    "VS",
                                                    "vs_5_0",
                                                    nullptr,
    ...
    // 创建顶点布局
    HR(device->CreateInputLayout(basicInstLayout,
                                 ARRAYSIZE(basicInstLayout),
                                 blob->GetBufferPointer(),
                                 blob->GetBufferSize(),
                                 pImpl->m_pInstancePosNormalTexLayout.GetAddressOf()));

    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("BasicObject_VS",
                                                    shader_path + L"BasicObject_VS.hlsl",
                                                    device,
                                                    "VS",
                                                    "vs_5_0",
                                                    nullptr,
    ...
    HR(device->CreateInputLayout(VertexPosNormalTex::inputLayout,
                                 ...
                                 blob->GetBufferPointer(),
                                 blob->GetBufferSize(),
                                 pImpl->m_pVertexPosNormalTexLayout.GetAddressOf()));

    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("NormalMapInstance_VS",
                                                    shader_path + L"NormalMapInstance_VS.hlsl",
                                                    device,
                                                    "VS",
                                                    "vs_5_0",
                                                    nullptr,
                                                    blob.ReleaseAndGetAddressOf()));
    // 创建顶点布局
    HR(device->CreateInputLayout(normalmapInstLayout,
                                 ARRAYSIZE(normalmapInstLayout),
                                 blob->GetBufferPointer(),
                                 blob->GetBufferSize(),
                                 pImpl->m_pInstancePosNormalTangentTexLayout.GetAddressOf()));

    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("NormalMapObject_VS",
                                                    shader_path + L"NormalMapObject_VS.hlsl",
                                                    device,
                                                    "VS",
                                                    "vs_5_0",
                                                    nullptr,
                                                    blob.ReleaseAndGetAddressOf()));
    // 创建顶点布局
    HR(device->CreateInputLayout(VertexPosNormalTangentTex::inputLayout,
                                 ARRAYSIZE(VertexPosNormalTangentTex::inputLayout),
                                 blob->GetBufferPointer(),
                                 blob->GetBufferSize(),
                                 pImpl->m_pVertexPosNormalTangentTexLayout.GetAddressOf()));

    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("DisplacementMapObject_VS",
                                                    shader_path + L"DisplacementMapObject_VS.hlsl",
                                                    device,
                                                    "VS",
                                                    "vs_5_0",
                                                    nullptr,
    ...
    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("DisplacementMapInstance_VS",
                                                    shader_path + L"DisplacementMapInstance_VS.hlsl",
                                                    device,
                                                    "VS",
                                                    "vs_5_0",
                                                    nullptr,
    ...
    // 创建外壳着色器
    //
    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("DisplacementMap_HS",
                                                    shader_path + L"DisplacementMap_HS.hlsl",
                                                    device,
                                                    "HS",
                                                    "hs_5_0",
                                                    nullptr,
    ...
    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("DisplacementMap_DS",
                                                    shader_path + L"DisplacementMap_DS.hlsl",
                                                    device,
                                                    "DS",
                                                    "DS_5_0",
                                                    nullptr,
    ...
    HR(pImpl->m_pEffectHelper->CreateShaderFromFile(
        "Basic_PS", L"Shaders\\Basic_PS.hlsl", device, "PS", "ps_5_0", nullptr, blob.ReleaseAndGetAddressOf()));

    HR(pImpl->m_pEffectHelper->CreateShaderFromFile("NormalMap_PS",
                                                    shader_path + L"NormalMap_PS.hlsl",
                                                    device,
                                                    "PS",
                                                    "ps_5_0",
                                                    nullptr,
    ...
    D3D11SetDebugObjectName(pImpl->m_pInstancePosNormalTangentTexLayout.Get(),
                            "BasicEffect.InstancePosNormalTangentTexLayout");
    D3D11SetDebugObjectName(pImpl->m_pVertexPosNormalTangentTexLayout.Get(),
                            "BasicEffect.VertexPosNormalTangentTexLayout");
...
void XM_CALLCONV BasicEffect::SetWorldMatrix(DirectX::FXMMATRIX W) { XMStoreFloat4x4(&pImpl->m_World, W); }

void XM_CALLCONV BasicEffect::SetViewMatrix(DirectX::FXMMATRIX V) { XMStoreFloat4x4(&pImpl->m_View, V); }

void XM_CALLCONV BasicEffect::SetProjMatrix(DirectX::FXMMATRIX P) { XMStoreFloat4x4(&pImpl->m_Proj, P); }

void XM_CALLCONV BasicEffect::SetShadowTransformMatrix(DirectX::FXMMATRIX S)
{
    XMMATRIX shadowTransform = XMMatrixTranspose(S);
    pImpl->m_pEffectHelper->GetConstantBufferVariable("g_ShadowTransform")
        ->SetFloatMatrix(4, 4, (const FLOAT*)&shadowTransform);
}

void BasicEffect::SetDirLight(size_t pos, const DirectionalLight& dirLight)
{
    pImpl->m_pEffectHelper->GetConstantBufferVariable("g_DirLight")
        ->SetRaw(&dirLight, (UINT)pos * sizeof(dirLight), sizeof(dirLight));
}

void BasicEffect::SetPointLight(size_t pos, const PointLight& pointLight)
{
    pImpl->m_pEffectHelper->GetConstantBufferVariable("g_PointLight")
        ->SetRaw(&pointLight, (UINT)pos * sizeof(pointLight), sizeof(pointLight));
}

void BasicEffect::SetSpotLight(size_t pos, const SpotLight& spotLight)
{
    pImpl->m_pEffectHelper->GetConstantBufferVariable("g_SpotLight")
        ->SetRaw(&spotLight, (UINT)pos * sizeof(spotLight), sizeof(spotLight));
}
...
void BasicEffect::SetSSAOEnabled(bool enabled)
{
    ...
    pImpl->m_pEffectHelper->GetEffectPass("BasicObject")
        ->SetDepthStencilState((enabled ? RenderStates::DSSEqual.Get() : nullptr), 0);
    pImpl->m_pEffectHelper->GetEffectPass("BasicInstance")
        ->SetDepthStencilState((enabled ? RenderStates::DSSEqual.Get() : nullptr), 0);
    pImpl->m_pEffectHelper->GetEffectPass("NormalMapObject")
        ->SetDepthStencilState((enabled ? RenderStates::DSSEqual.Get() : nullptr), 0);
    pImpl->m_pEffectHelper->GetEffectPass("NormalMapInstance")
        ->SetDepthStencilState((enabled ? RenderStates::DSSEqual.Get() : nullptr), 0);
    pImpl->m_pEffectHelper->GetEffectPass("DisplacementMapObject")
        ->SetDepthStencilState((enabled ? RenderStates::DSSEqual.Get() : nullptr), 0);
    pImpl->m_pEffectHelper->GetEffectPass("DisplacementMapInstance")
        ->SetDepthStencilState((enabled ? RenderStates::DSSEqual.Get() : nullptr), 0);
}
