...
float4 PS(PassThoughVertexOut ptIn,out float depthOut:SV_Depth) : SV_Target
{
    ...
    float eyeZ = g_DepthTexture.Sample(g_TexSampler, uvCoord.x).x;
    if (eyeZ >= 0.0f)
        discard;
 
    //reconstruct eye space form depth
    float3 eyePos = ndcToEyeSpace(uvCoord, eyeZ);
    
    //finite difference approx for normals,
    float3 zl = eyePos - ndcToEyeSpace(uvCoord - float2(g_InvTexScale.x, 0.0), g_DepthTexture.Sample(g_TexSampler, uvCoord - float2(g_InvTexScale.x, 0.0)).x);
    float3 zr = ndcToEyeSpace(uvCoord + float2(g_InvTexScale.x, 0.0), g_DepthTexture.Sample(g_TexSampler, uvCoord + float2(g_InvTexScale.x, 0.0)).x) - eyePos;
    float3 zt = ndcToEyeSpace(uvCoord + float2(0.0, g_InvTexScale.y), g_DepthTexture.Sample(g_TexSampler, uvCoord + float2(0.0, g_InvTexScale.y)).x) - eyePos;
    float3 zb = eyePos - ndcToEyeSpace(uvCoord - float2(0.0, g_InvTexScale.y), g_DepthTexture.Sample(g_TexSampler, uvCoord - float2(0.0, g_InvTexScale.y)).x);
    ...
    float4 worldPos = mul(g_WorldViewInv, float4(eyePos, 1.0f));
    
    //TODO:SHADOW
    
    float3 l = mul(g_WorldView, float4(g_DirLight[0].Direction, 0.0f)).xyz;
    ...
    float3 rWorld = mul(g_WorldViewInv, float4(rEye, 0.0f)).xyz;
   ...
   float4 clipPos = mul(g_Proj, float4(0.0f, 0.0f, eyeZ, 1.0f));
    clipPos.z /= clipPos.w;
    depthOut = clipPos.z;
